...
WORKDIR /src
COPY ["og-image/og-image.csproj", "og-image/"]
RUN dotnet restore "og-image/og-image.csproj"
COPY . .
WORKDIR "/src/og-image"
RUN dotnet build "og-image.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "og-image.csproj" -c Release -o /app/publish /p:UseAppHost=false
